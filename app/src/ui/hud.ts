...
export function startHud(controls: FlyControls, frame: LocalFrame, terrain: Terrain): void {
  const el = document.getElementById("hud")!;
  setInterval(() => {
    const p = controls.camera.position;
    const ll = frame.toLonLat(p.x, p.z);
    const ground = terrain.elevationAt(ll);
    const depth = ground === null ? null : ground - p.y;
    ...
    el.innerHTML =
      ...
      `<div>海抜 T.P. ${p.y >= 0 ? "+" : ""}${p.y.toFixed(1)} m</div>` +
